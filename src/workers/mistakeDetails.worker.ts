...
self.onmessage = (evt: MessageEvent) => {
  ...
  for (const [gameId, group] of allEntries) {
    ...
    // Attempt to parse PGN; if it fails, fall back to emitting minimal items
    let verbose: VerboseMove[] | null = null
    try {
      const engine = new Chess()
      // enable sloppy to tolerate comments/annotations
      engine.loadPgn(pgnRaw, { sloppy: true } as any)
